...
**09-22 재발**: 04:33 재무장한 백그라운드 감시가 유휴 중 메모리 부족으로 또 강제종료(06:0x 확인). 여유 메모리 04:29 21,533MB → 06:04 2,540MB.
강제종료돼도 watch8.sh 의 bash 3·tail 2 가 주인 없이 남는다 → 매번 ps 로 찾아 kill. 형께 "말씀 있을 때까지 꺼둔다" 보고함(이틀 연속).
메모리를 누가 먹는지는 미조사.

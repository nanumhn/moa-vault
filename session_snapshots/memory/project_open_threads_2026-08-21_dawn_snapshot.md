...
**갱신 2026-08-21 14:25 KST** (04시 최초 → 오전 작업분 반영 → 리셋 전 저장)
...
## 이번 배포에 들어간 것 — ★미배포 실측 확인(`/zodiac` 404, PR OPEN) (브랜치 `feat/zodiac`, **커밋 7개**, HEAD `17e3511`)
...
## 리셋 전 추가 발견 (2026-08-21 오전)
- **`moa_moc_staleness.ps1`이 통과 오진을 냈다.** 종료코드 0(3일 이내)인데 `k-saju MOC.md`는 `updated: 2026-08-20`으로 **오늘 작업이 하나도 반영 안 된 상태**였다. 스크립트가 **시간만 보고 내용 반영 여부는 안 본다.** 형이 8/20 지적한 "낡은 내용으로 업무 정리" 문제가 재현될 뻔했다. → 스크립트에 "최근 커밋 대비 MOC 언급 여부" 같은 내용 기준을 넣는 게 다음 과제.
- 저장 cron이 예약 13:55보다 **30분 늦은 14:25에 발동**(8월 내내 같은 지연). 리셋 스크립트가 35분 대기하므로 아슬아슬하다.

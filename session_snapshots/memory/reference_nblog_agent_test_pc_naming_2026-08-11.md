...
metadata:
  ...
  modified: 2026-08-11T04:02:57.325Z
---
...
- **개발1 PC** — 형 집 PC. 맨 처음 nblog-agent 0.1.0 설치→크래시 재현했던 그 PC(사용자명 `smd`로 확인됨).
- **개발2 PC** — 형이 그다음 테스트로 옮겨간 다른 PC("2번 PC"). 개발1 PC에서 했던 크롬 디버그포트 워크어라운드(`--remote-debugging-port=9335 --no-sandbox`)를 여기서도 별도로 다시 실행해야 함 — PC마다 독립적인 환경이라 한쪽에서 한 조치가 다른 쪽에 안 넘어감.

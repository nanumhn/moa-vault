...
## 2026-09-19 02:1x 갱신 — **재발했다. 열흘 만이고, 모양이 조금 다르다**

[확인: 트랜스크립트 mtime + `mcp_guard.log`·`session_reset.log` 직접 조회]
...
| `6af42f20` | 02:07:24 | (생존) | — |

**09-09과 다른 점**: 첫 사망은 유휴 단시간이 아니라 **11시간 생존 뒤**였다. 그런데 **재생성분 두 개가 1분·3분 만에 연속으로 죽었다** — 부트스트랩을 실행하던 도중이다(둘 다 opus 출력 2.6k·8.4k 토큰, 도구시간 2.7초·2.5초). **'유휴라서 죽는다'는 09-09의 관찰은 이번 표본으로 깨진다.**

**새로 확인한 것**
- **가드가 21:16~00:56 4시간 내내 `BLIND`** — `3 bun process(es) visible but no CommandLine readable (not elevated?)`. 이 상태에서 가드는 아무것도 안 한다. 그래서 **사망 시각을 못 잡는다.** ★이 BLIND 구멍 자체가 별개의 결함이다.
- **죽은 세션 둘 다 마지막 레코드가 `cost-state`** → claude가 **정상 shutdown 경로를 밟았다.** 크래시가 아니다.
- 메모리 부족 아님 [확인: 여유 13.17GB · 커밋 72.3%]. 이벤트로그 8시간치 claude/node/bun 매치 0건.
- respawn 스크립트가 죽인 것 아님 — 세 번 다 kill 직전 로그가 `no claude.exe running`.

**진단기 재가동** pid 21940, 첫 줄 `02:15:04 | claude=1 [35792<-30412] | wt=1 [30412]`. 판독법은 위와 동일.

★**함정 재발**: 진단기 실행 여부를 `CommandLine -match 'death_probe'`로 셌더니 `1` — **조회 명령 자신이 매치**. `$PID` 제외하니 `count=0`. **이 문서 맨 아래 적혀 있는 그 함정을 읽고도 그대로 밟았다.**

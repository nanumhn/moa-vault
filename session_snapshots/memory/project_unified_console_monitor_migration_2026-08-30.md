...
- **★2026-09-16 14:1x 재확인(관리자 API `/api/manager/jobs` 직접 조회)**: 이관 잡이 13개→**16개**로 늘었고 전부 `lastExitCode:0`. 신규 확인분 = `moa-nblog-stack-up`·`moa-worker-monitor`, 그리고 **`moa-shared-rules-watch`는 jobs.json에 들어왔지만 `enabled:false`** (08-30에 "다음 점검 때 재확인"으로 남겼던 질문의 답 — 등록은 됐고 꺼져 있다). 스크립트 자체 결함(SECTION-A 마커 소실)은 형 판단 "재시작 이후에 고치자" 그대로 미수리로 추정, 이번엔 실행해 보지 않음. 다리 2개(`moa-dex-bridge`·`moa-jena-bridge`)는 `status:running`.
- **작업스케줄러 쪽 표기(같은 시각 `Get-ScheduledTaskInfo` 직접 조회)**: `MoaDiscordWatchdogExternal`·`MoaAtzReportWatchdogExternal`·`MoaWorkChannelWatchdogExternal`·`MoaCommandChannelWatchdogExternal`·`MoaIssueForumWatchdogExternal` 5개 모두 **Disabled·마지막 실행 2026-08-30**인데 로그 파일은 계속 늘어난다(workchannel 로그 14:13 줄). 설계대로다 — 콘솔이 대체 실행 중. 세션마다 이걸 "감시가 죽었다"로 오독하는 일이 반복되므로(09-16 오전 스냅샷에도 "미확인"으로 남아 있었음) **Disabled 표기만 보고 고장으로 판단하지 말 것.** 재활성화는 여전히 금지 [[feedback_never_run_disabled_scheduler_tasks_2026-09-07]].

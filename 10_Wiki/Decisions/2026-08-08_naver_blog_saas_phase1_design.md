...
작성: 윤서진(CTO) · 2026-08-08 · **rev5 (qa-lead-jian 4차 검수 FAIL 반영)**
선행 결정문서: [[2026-08-08_naver_blog_saas_plan]] (형 결재 완료 · 노선 ㉡ 사용자 PC 설치형 확정)
상태: **설계 초안 rev5 — qa-lead-jian 5차 검수 대기**
...
- **3장을 "규범 정의(3-A) / 근거(3-B) / 검증잡(3-C)"로 재작성.** 간격·쿼터·슬롯·재시도·지연흡수의 정의는 **3-A에만** 존재하고, 4·5·7·8장은 규칙 ID(`G1`·`G2` 게이트, `SL1`~`SL5` 슬롯, `R1` 리스만료, `DF1`~`DF3` 지연흡수, `J1`·`J2` 지터, `INV1`~`INV4` 불변식, `L1`~`L4` 방어층)를 **참조만** 한다.
...
동시성 방어는 **예약(`reservedUntil`, 규칙 `SL2`)** 이 담당한다. 잡 수령 시점에 자리를 배타적으로 임대하므로 같은 자리에 두 잡이 동시에 들어가지 못한다. **소모(`consumedAt`, 규칙 `SL3`)는 이와 별개로 "발행 버튼을 실제로 누른 시점"에만 찍히고**, 그 값은 `INV4`에 의해 `publishAttemptAt`의 파생값으로만 존재한다.

> **[4차 반려·중대] 이 문단의 rev3 원문을 폐기했다.** 원문은 "`consumedAt`을 잡 수령 시점에 잡기 때문에 중복 방어가 더 강해진다"였는데, ① rev4의 `SL3`(발행 버튼 시점)와 정면 충돌하고 ② "더 강해진다"는 주장 자체를 2-4의 [F4] 문단에서 이미 철회했다(DB 제약 7이 없으면 성립하지 않는 주장이었다). 예약과 소모를 분리한 지금은 **동시성=`SL2`, 쿼터=`SL3`+`INV4`, 최후 방어=DB 제약 7**로 역할이 갈린다.
...
model Blog {
  ...
  //      발행 성공 시 0 리셋. 4 초과(=이틀 무발행)면 경보 + 사용자 알림.
...
-- 7) ★[F4] 한 자리에서 실제로 발행된 잡은 최대 1건 = 불변식 `INV2`.
...
> **[F4] 제약 7과 앱 로직의 역할 분담 (정직하게)**: 제약 7은 `publishAttemptAt`을 UPDATE하는 순간, 즉 **에이전트가 이미 발행 버튼을 누른 뒤에** 위반을 잡는다. 따라서 이건 *실시간 예방*이 아니라 **불변식 위반을 시끄럽게 터뜨리는 최후 방어선**이다. 실시간 예방은 3-A 규칙 `SL2`(예약)가 담당한다. 제약 7이 발동하면 그 자체가 S1에 구멍이 있다는 뜻이므로 A1에 즉시 경보를 띄운다. rev3은 이 역할 분담을 안 밝히고 "중복 방어가 더 강해졌다"고만 썼는데, 그 주장은 제약 7이 없으면 성립하지 않았다.
...
| **`INV4`** | **`slot.consumedAt IS NOT NULL` ⟺ 그 슬롯에 `publishAttemptAt IS NOT NULL`인 잡이 존재** | 앱(`SL3`·`SL4`) + `integrity-check` |
...
| **`SL3` 소모** | **`publishAttemptAt`을 세팅하는 모든 트랜잭션**은 같은 트랜잭션에서 `consumedAt = now()`, `consumedByJobId = <job>`을 세팅한다(`INV4` 유지). 경로는 두 개다 — 정상 발행(`PUBLISH_SUBMITTED` 수신)과 `R1-a`(리스만료 보수 처리). |
| **`SL4` 해제** | **`publishAttemptAt`이 NULL이 되는 모든 트랜잭션**은 같은 트랜잭션에서 `INV4`를 재평가해 자리를 되돌린다. 구체적으로 잡이 `TERMINAL`이고 그 슬롯에 `publishAttemptAt IS NOT NULL`인 잡이 **하나도 없으면** → `reservedByJobId`·`reservedUntil`·`consumedAt`·`consumedByJobId`를 **전부 NULL**로. 경로는 두 개다 — ⓐ 애초에 발행 못 한 종료(`SKIPPED`·`EXPIRED`·`CANCELED`·`FAILED`: `consumedAt`이 원래 NULL이라 예약만 풀림) ⓑ **`R1-c`(검증 결과 미발행 확정)**: `publishAttemptAt`이 지워지므로 `consumedAt`도 같이 풀린다. |
| **`SL5` 재사용** | 해제된 자리에는 `attemptSeq + 1` 잡을 새로 만들 수 있다(재시도·수동 발행). |

```sql
-- SL4 해제: publishAttemptAt이 사라지는 트랜잭션 끝에서 INV4를 재평가한다.
--   ⓐ 경로에선 consumedAt이 원래 NULL이라 사실상 예약만 풀리고,
--   ⓑ R1-c 경로에선 consumedAt까지 같이 풀려 자리가 완전히 되살아난다.
...
> **`R1-a` → `R1-c` 왕복이 정상 동작하는지가 이 설계의 급소다**(4차 반려 치명). `R1-a`가 자리를 CONSUMED로 만들었다가 `R1-c`가 되돌리는 경로이므로, `SL4`가 `consumedAt`을 안 건드리면 자리가 영구 소각된다. 회귀 테스트 2개를 8장에 못 박았다 — **① `R1-c` 이후 재시도 잡이 실제로 `SL2`를 통과해 claim되는지 ② `G2`가 그 자리를 더 이상 세지 않는지.**

#### 3-A-6. 지연 흡수 규칙 DF1~DF3
...
| **`DF4`** | **발행 없이 끝난 모든 슬롯**(`SKIPPED`·`EXPIRED`·`R1-c`로 미발행 확정)마다 `blog.consecutiveUnpublishedSlots += 1`. 발행이 한 번이라도 성공하면 0으로 리셋. **4를 넘으면**(= 만 이틀 무발행) A1 경보 + 사용자 알림 |
...
| `job-reaper` | 1분 | 리스 만료 처리(`R1-a`/`R1-b`) / `expiresAt` 초과 잡 `EXPIRED` + 알림 / 지연 흡수 재예약(`DF1`~`DF2`) / 종료 잡의 자리 해제(`SL4`) |
| `agent-watch` | 1분 | 오프라인 전환 감지 → 알림(6시간 쿨다운) |
| `ai-draft` | 1시간 | 업셀 사용자 글감 부족분 로컬 LLM 배치 생성 |
| `integrity-check` | 일 1회 | `INV1` 위반(RISKY인데 `publishAttemptAt` NULL) + **`INV4` 위반(`consumedAt`과 실제 발행 흔적 불일치)** 탐지 |
...
3. 테스트 우선순위(Vitest) — **하드 룰부터 테스트로 못 박는다**:
   ...
   - **[F6 ★치명 회귀] `R1-a` → `R1-c` 왕복**(4차 검수 발견): 리스만료로 `consumedAt`이 찍힌 자리가 검증 `NOT_FOUND` 후 **완전히 되살아나는지**를 세 각도로 —
   ...
   - **[F7] 무발행 감지**: 매 슬롯이 `SKIPPED`로만 끝나는 블로그에서 `consecutiveUnpublishedSlots`가 증가해 **4에서 경보가 뜨는지**(`DF4`). `DF3`만으로는 안 잡히는 것도 함께 확인
   ...
   - **[D3] DB 제약**: **7개 존재 확인**(CHECK 6 + 부분 유니크 인덱스 1 — 개수를 상수로 박고 grep)

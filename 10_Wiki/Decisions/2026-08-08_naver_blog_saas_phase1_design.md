...
- **★[전제조건 미충족 → 2026-08-08 해소]** `INV6`(블로그 단위 배타)는 **`job-reaper`가 있어야 안전한 방어**였다. reaper가 없던 동안은 걷히지 않은 `CLAIMED` 잡 하나가 **그 블로그의 후속 발행을 무기한 막았다**(`expiresAt` 3h 상한도 `EXPIRED` 전이를 reaper가 하므로 자동 발동하지 않았다). 즉 "중복 발행은 확실히 막히지만, 한 번 막히면 스스로 못 푸는" 상태였다. **`job-reaper` 구현으로 해소됐고, 상태값이 아니라 `claim` 결과로 확인했다**(5-3-1 회귀 `F9-②`).
...
- 테스트 **101개**(신규 31) 전부 실제 Postgres 통과. **뮤테이션 16종을 넣어 전부 테스트가 죽는 것을 확인**했다(초기 2종은 살아남아서 — 검증 잡 `ALREADY_ACTIVE` 가드와 후보창 기아 — 그 자리에 테스트를 새로 만들고 다시 죽는 것을 확인했다. 살아남은 뮤테이션을 그대로 두고 "통과했다"로 끝내지 않았다).
...
3. 테스트 우선순위(Vitest) — **하드 룰부터 테스트로 못 박는다**:
   ...
   - **[F9 ★자가복구 회귀] `job-reaper` 11종**(5-3-1) — "어떤 상태도 사람 손 없이 스스로 풀린다"를 못 박는다:
     ...
     · **⑪★ DB 세션 타임존**: ORM이 쓴 값과 raw `now()`가 같은 시계인지 / 세션이 UTC가 아니면 reaper가 **던지는지**

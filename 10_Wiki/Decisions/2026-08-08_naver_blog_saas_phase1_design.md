...
- **★[전제조건 미충족 — 명시]** `INV6`(블로그 단위 배타)는 **`job-reaper`가 있어야 안전한 방어**다. 지금은 reaper가 없어서 걷히지 않은 `CLAIMED` 잡 하나가 **그 블로그의 후속 발행을 무기한 막는다**(`expiresAt` 3h 상한은 `EXPIRED` 전이를 reaper가 하므로 자동 발동하지 않는다 — 3-A-5 아래 잔존 리스크 참조). 즉 현재 조합은 "중복 발행은 확실히 막히지만, 한 번 막히면 스스로 못 푸는" 상태다. **`job-reaper`가 다음 작업 1순위인 이유가 이것이다.**
...
| `job-reaper` | 1분 | 리스 만료 처리(`R1-a`/`R1-b`) / `expiresAt` 초과 잡 `EXPIRED` + 알림 / 지연 흡수 재예약(`DF1`~`DF2`) / **`SL4-SWEEP` 자가복구**(트리거 ⓩ — `INV4`가 깨진 슬롯을 매분 되돌림) |

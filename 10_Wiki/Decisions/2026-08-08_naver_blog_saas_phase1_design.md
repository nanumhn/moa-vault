...
작성: 윤서진(CTO) · 2026-08-08 · **rev4 (qa-lead-jian 3차 검수 FAIL 반영 — 발행제약 전면 재작성)**
선행 결정문서: [[2026-08-08_naver_blog_saas_plan]] (형 결재 완료 · 노선 ㉡ 사용자 PC 설치형 확정)
상태: **설계 초안 rev4 — qa-lead-jian 4차 검수 대기**
...
- **3장을 "규범 정의(3-A) / 근거(3-B) / 검증잡(3-C)"로 재작성.** 간격·쿼터·슬롯·재시도·지연흡수의 정의는 **3-A에만** 존재하고, 4·5·7·8장은 규칙 ID(`G1`,`G2`,`S1`~`S5`,`R1`,`D1`~`D3`,`J1`,`J2`,`INV1`~`INV3`)를 **참조만** 한다.
...
| **②** [치명] | `SKIPPED`가 자리 해제 목록에 없어 **자리 영구 소각** | `S4`의 조건을 "`TERMINAL` ∧ `publishAttemptAt IS NULL`"로 정의하고 **`TERMINAL`에 `SKIPPED` 포함**을 명시. 예약(`reservedUntil`)과 소모(`consumedAt`)를 컬럼 수준에서 분리해 애초에 소각이 불가능하게 만듦 |
| **③** [치명] | "이미 점유돼 있으면" 판정 기준 미정의 → 재수령 죽거나 중복발행 | 두 가지를 동시에 답함: `S2`에 `OR reservedByJobId = $jobId`(자기 잡 갱신 허용)를 명시하고, **`R1`으로 재수령 자체를 금지**. 리스 만료는 "결과 미상"으로 보고 검증 잡으로만 구제 → 중복발행 경로 소멸 |
...
| **⑧** [경미] | 구스키마 잔존(`slotDate/slotIndex가 NULL`) + `PublishSlot` 생성주체·시점 미정의 | `slotId IS NULL`로 정정. **`S1`에 생성 주체·시점 명시**(블로그 타임존 매일 00:05, `slot-planner`, `plannedAt`은 지터 적용 전) |
...
| | 내용 |
|---|---|
| **확정(승인 대상)** | ① DB 스키마 **모델 17개** + raw SQL 제약 **7개**(CHECK 6 + 부분 유니크 인덱스 1) ② 웹 대시보드 **화면 13개** ③ PC 에이전트↔서버 인터페이스 **엔드포인트 7개** ④ 발행 제약(하루 2회·12시간) 강제 지점 4곳 + 근거 계산 |
...
model Blog {
  ...
  // [F5] 지연 흡수가 연속으로 몇 슬롯째 걸렸나. 정시 발행이 한 번 나오면 0으로 리셋.
  //      드리프트는 슬롯당 40분씩 줄므로 최악(3시간)에도 4를 넘지 않는다 → 8 초과는 버그 신호.
  //      per-job 카운터로는 체인 이상을 절대 못 잡는다(rev3의 죽은 방어) → 블로그 단위로 둔다.
...
model PublishSlot {
  ...
  // ★[F2/F3] 예약(reservation)과 소모(consumption)는 서로 다른 개념이다. 3-A 규칙 S1~S4.
...
model PublishJob {
  ...
  //      Blog.consecutiveDeferredSlots가 담당한다(3-A 규칙 D3). rev3은 이걸 혼동해
...
-- ─── 부분 유니크 인덱스 1개 (CHECK로는 표현 불가) ───

-- 7) ★[F4] 한 자리에서 실제로 발행된 잡은 최대 1건.
...
> **[E2] rev2에서 바뀐 점**: rev2의 CHECK#2는 `PublishJob`에 `slotDate`/`slotIndex`를 강제해 **블로그·날짜당 PUBLISH 행이 영구히 2개로 고정**됐고, `EXPIRED` 후 수동 발행·재시도가 unique violation으로 막혔다. rev4는 쿼터를 `PublishSlot`으로 옮겨 **자리는 2개로 유지되면서 시도는 여러 번 가능**하고, 제약 7로 **"자리당 실제 발행 1회"를 DB가 다시 보장**한다.

> **[F4] 제약 7과 앱 로직의 역할 분담 (정직하게)**: 제약 7은 `publishAttemptAt`을 UPDATE하는 순간, 즉 **에이전트가 이미 발행 버튼을 누른 뒤에** 위반을 잡는다. 따라서 이건 *실시간 예방*이 아니라 **불변식 위반을 시끄럽게 터뜨리는 최후 방어선**이다. 실시간 예방은 3-A 규칙 S1(예약)이 담당한다. 제약 7이 발동하면 그 자체가 S1에 구멍이 있다는 뜻이므로 A1에 즉시 경보를 띄운다. rev3은 이 역할 분담을 안 밝히고 "중복 방어가 더 강해졌다"고만 썼는데, 그 주장은 제약 7이 없으면 성립하지 않았다.

> `prisma migrate dev` 로 생성된 SQL 파일 끝에 위 **7개(CHECK 6 + 부분 유니크 인덱스 1)** 를 손으로 덧붙이고, 이후 `prisma migrate diff` 로 드리프트가 안 나는지 확인한다. **CI에 "제약 7개 존재 확인" 테스트를 넣는다** — 나중에 누가 마이그레이션을 재생성하면 조용히 사라지는 종류의 방어이기 때문이다.
...
- **[미검증]** 위 **7개 문(ALTER 6 + CREATE INDEX 1)** 을 **실제 Postgres에서 실행해 보지는 못했다.** 로컬에 Postgres 컨테이너·이미지가 없고 docker pull이 막혀 있다. 따라서 "3번째 슬롯 INSERT가 거부된다", "한 자리에 두 번째 발행이 거부된다"는 것은 아직 **설계상 주장이지 실행 확인이 아니다.** 페이즈2 첫 마이그레이션에서 반드시 실측하고, 8장 테스트 목록이 그 확인을 담당한다. (1차 설계에서 "물리적으로 안 생긴다"고 단정했다가 반려된 것과 같은 실수를 반복하지 않기 위해 명시한다.)
...
| 값 | 언제 | 사용자에게 보이는 문장 |
|---|---|---|
| `MIN_INTERVAL_12H` | `G1` 거짓이고 `D2` 지연 흡수도 불가 | "직전 글과 너무 가까워 이번 발행을 건너뛰었습니다" |
...
> ### ★ 이 문서를 고칠 사람에게
> **간격·쿼터·슬롯·재시도·지연흡수의 정의는 오직 3-A에만 있다.** 3-B는 "왜 이렇게 됐나"(근거·이력)이고, 4·5·7·8장은 규칙 ID(`G1`,`S2`,`R1`…)를 **참조만** 한다.
...
슬롯 상태 FREE → RESERVED → CONSUMED     (CONSUMED는 되돌아오지 않는다, 예외는 R1-c)
```

| 불변식 | 내용 | 지키는 주체 |
|---|---|---|
| `INV1` | `status ∈ RISKY` ⟺ `publishAttemptAt IS NOT NULL` | 앱 + `integrity-check` 일배치 |
| `INV2` | 한 슬롯에 `publishAttemptAt`이 찍힌 잡은 **최대 1건** | **DB 제약 7**(2-4) |
...
> **동시 실행 배타는 별도 술어를 두지 않는다.** rev3에는 "`CLAIMED`/`RUNNING`이 있으면 차단"하는 세 번째 쿼리가 따로 있었는데, 슬롯 예약(`S2`)이 같은 일을 더 정확히 한다. **중복 서술을 없애기 위해 그 쿼리는 삭제한다.**

#### 3-A-4. 슬롯 규칙 S1~S5

| ID | 규칙 |
|---|---|
| **`S1` 생성** | `slot-planner`가 **블로그 타임존 기준 매일 00:05**에 그날의 슬롯 행을 만든다. `enabled`이고 `weekdays`가 그날과 맞는 `Schedule` 하나당 1행. `plannedAt = 그날 날짜 + hour:minute`(타임존 적용, **지터 적용 전** — `G1` 규칙A의 기준이므로). 수동 발행 시 해당 자리 행이 없으면 그 시점에 생성한다(비어 있는 `slotIndex` 사용). |
| **`S2` 예약** | 잡 수령 시 아래 UPDATE가 **1행을 반환할 때만** 잡을 내준다. 0행이면 내주지 않는다. |
| **`S3` 소모** | `publishAttemptAt`을 기록하는 **바로 그 트랜잭션에서** `consumedAt = now()`, `consumedByJobId` 세팅. 영구. |
| **`S4` 해제** | 잡이 `TERMINAL`에 도달했고 `publishAttemptAt IS NULL`이면 예약을 푼다(`reservedByJobId`/`reservedUntil` → NULL). **`TERMINAL`에는 `SKIPPED`가 포함된다** — 글이 안 올라갔는데 자리를 소각하면 안 되기 때문(3차 반려 ②). `consumedAt`은 애초에 안 찍혔으므로 손대지 않는다. |
| **`S5` 재사용** | 해제된 자리에는 `attemptSeq + 1` 잡을 새로 만들 수 있다(재시도·수동 발행). |

```sql
-- S2 예약: 원자적 UPDATE. 반환 행이 0이면 그 잡은 수령 불가.
UPDATE "PublishSlot"
 ...
 WHERE "id" = $slotId
   ...
   AND ( "reservedUntil" IS NULL
      ...
      OR "reservedByJobId" = $jobId )     -- ★자기 잡의 리스 갱신은 허용
RETURNING "id";
```
...
| ID | 상황 | 처리 |
|---|---|---|
| `R1-a` | 리스 만료 시 상태가 `CLAIMED`/`RUNNING`/`SUBMITTED` | `publishAttemptAt = claimedAt`(보수적으로 가장 이른 가능 시각)로 채우고 `UNVERIFIED`로 전이 → `S3`로 슬롯 CONSUMED → 검증 잡 생성(3-C). `INV1` 유지 |
| `R1-b` | 리스 만료 시 상태가 `QUEUED`(한 번도 수령 안 됨) | `EXPIRED`. 에이전트가 손댄 적 없으므로 발행 가능성 0 → `publishAttemptAt` NULL 유지, `S4`로 자리 해제. 검증 불필요 |
| `R1-c` | 검증 결과 `NOT_FOUND` | **`publishAttemptAt`을 NULL로 되돌리고** `FAILED`로 전이 → `S4`로 자리 해제 → 재시도 가능. **문서 전체에서 `publishAttemptAt`을 지우는 유일한 경로**이며, 근거(검증으로 미발행이 확인됨)를 `AuditEvent`에 남긴다 |

#### 3-A-6. 지연 흡수 규칙 D1~D3

| ID | 규칙 |
|---|---|
| `D1` | `G1(blog, scheduledAt)`이 거짓이면 `deferTo = lastAttemptAt + DEFER_OFFSET` 을 계산한다 |
| `D2` | `deferTo ≤ expiresAt`이면 `scheduledAt = deferTo`, `job.deferCount += 1`(상한 3), `blog.consecutiveDeferredSlots += 1`. 아니면 `SKIPPED(MIN_INTERVAL_12H)` + `S4` |
| `D3` | 흡수 없이 정시 발행이 한 번 성공하면 `blog.consecutiveDeferredSlots = 0`. **8을 넘으면** A1 경보 + 해당 블로그 자동 일시중지 |

> **`D3`이 블로그 단위인 이유(3차 반려 ⑤ 답)**: 드리프트 체인은 **슬롯마다 다른 잡**으로 이어지므로 `PublishJob.deferCount`(per-job)는 항상 1에서 끝난다 — rev3이 상한을 2에서 6으로 올린 것은 **더 도달 불가능하게 만든 것**이지 수정이 아니었다. 체인 이상은 잡을 가로지르는 누적 카운터로만 잡을 수 있어서 `Blog`로 옮겼다. 드리프트는 슬롯당 40분씩 줄어 최악(3시간)에도 4를 안 넘으므로, **8 초과는 감쇠가 깨졌다는 뜻 = 실제로 발동하는 방어**다.
...
| `L2` | `slot-planner`가 잡 생성 | `G1(blog, `**`그 잡의 scheduledAt`**`)` ∧ `G2` | `D1`→`D2` |
| `L3` | DB INSERT/UPDATE | `INV2` ∧ `INV3` | 예외 발생 + A1 경보 |
| `L4` | 잡 수령(claim) | `G1(blog, `**`now()`**`)` ∧ `G2` ∧ `S2` | `D1`→`D2`, 또는 잡 미발급 |
...
| `EXPIRED`/`SKIPPED` 후 재시도·수동 발행 | 자리 해제되어 `attemptSeq+1`로 정상 생성 | `S4`,`S5` |
...
| `NOT_FOUND` | `R1-c` — `publishAttemptAt`을 NULL로 되돌리고 `FAILED`(errorCode=`VERIFY_NOT_FOUND`) | **`S4`로 해제** → 재시도 가능 | "발행 안 됨, 재시도" |
...
- 1차 설계의 "±12시간 회색 처리(13시간 등 허용)" 방식은 **날짜 경계 간격이 11시간이 되어 매일 한 번이 죽으므로 폐기**(3-B-1).
...
```
- **리스(lease)**: 수령 시 `LEASE`(15분) 임대, 진행 이벤트로 갱신(`S2`의 자기 잡 재예약 경로). **에이전트가 죽어도 같은 잡을 다시 내주지 않는다** — 리스 만료는 `R1`이 처리한다(`R1-a` 결과 미상 → `UNVERIFIED` → 검증 잡 / `R1-b` 미수령이면 `EXPIRED` + 자리 해제). rev3까지 있던 "만료 후 재수령" 문구는 **`R1`과 정면 충돌하는 중복 서술이라 삭제했다**(3차 반려 ③).
- 서버는 이 시점에 `L4`를 수행한다 — `G1(blog, now())` ∧ `G2` ∧ `S2`. 위반 시 `D1`~`D2`(지연 흡수) 또는 `SKIPPED` 후 위 `skipped` 배열로 사유를 알린다. **판정 로직은 3-A에만 있고 여기서는 호출만 한다.**
...
- 서버는 `PUBLISH_SUBMITTED` 수신 시 **`publishAttemptAt`을 기록한다**(규칙 S3).
...
| `job-reaper` | 1분 | 리스 만료 처리(`R1-a`/`R1-b`) / `expiresAt` 초과 잡 `EXPIRED` + 알림 / 지연 흡수 재예약(`D1`~`D2`) / 종료 잡의 자리 해제(`S4`) |
| `agent-watch` | 1분 | 오프라인 전환 감지 → 알림(6시간 쿨다운) |
| `ai-draft` | 1시간 | 업셀 사용자 글감 부족분 로컬 LLM 배치 생성 |
| `integrity-check` | 일 1회 | `SUBMITTED/UNVERIFIED/VERIFIED`인데 `publishAttemptAt`이 NULL인 잡 탐지(INV1 위반) |
...
| **직전 발행이 늦어져 12시간 룰에 걸림** | **지연 흡수** `D1`~`D2` — 드리프트가 슬롯당 40분씩 줄어 최대 4슬롯 안에 정시 복귀(근거 3-B-4) | "발행이 10:50으로 조정되었습니다" + 며칠 내 원래 시각 복귀 |
| **슬롯이 `EXPIRED`된 뒤 재시도·수동 발행** | `S4`로 자리가 해제돼 `attemptSeq+1` 잡이 정상 생성(`S5`). rev2에서 unique violation으로 막히던 경로 | 이력에서 "재시도" 버튼이 실제로 동작 |
...
2. 마이그레이션 순서: NextAuth 테이블 → 도메인 테이블 → **2-4의 raw SQL 제약 7개(CHECK 6 + 부분 유니크 인덱스 1) 수동 삽입** → `Plan` 시드.
3. 테스트 우선순위(Vitest) — **하드 룰부터 테스트로 못 박는다**:
   - **[F0 ★구조] 단일 정의 소스 유지 검사** — 이번 반려의 근본원인(로직 중복 서술)이 재발하지 않게 CI로 못 박는다:
     · `G1`·`G2`·`S2`의 SQL 문자열이 코드베이스에 **각각 1곳에만** 존재하는지(중복 리터럴 검출 시 실패)
   ...
   - **[F2] 자리 소각 금지**: `SKIPPED`로 끝난 잡의 자리가 **해제되는지**(`S4`) / `SKIPPED` 후 같은 날 재시도가 가능한지
   - **[F3] 재수령 금지**: 리스 만료된 `CLAIMED`/`RUNNING`/`SUBMITTED` 잡이 **다시 claim되지 않고** `UNVERIFIED`로 가는지(`R1-a`) / `QUEUED`였던 잡은 `EXPIRED`+해제인지(`R1-b`)
   - **[F4] DB 보장**: 한 자리에 두 번째 `publishAttemptAt` UPDATE가 **거부되는지**(제약 7) / 하루 3번째 슬롯 INSERT 거부
